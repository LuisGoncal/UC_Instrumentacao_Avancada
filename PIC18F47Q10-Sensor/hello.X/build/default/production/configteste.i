...
# 3 "configteste.asm" 2
__CONFIG _CONFIG1, _FEXTOSC_OFF & _RSTOSC_LPRC & _CLKOUTEN_OFF & _CSWEN_OFF & _FCMEN_OFF & _MCLRE_ON & _WDTE_OFF & _LVP_OFF
__CONFIG _CONFIG2, _STVREN_OFF & _BORV_19 & _BORPD_BOHV & _LVP_OFF & _PWMPIN_OFF
__CONFIG _CONFIG3, _MSSPMSK_MSK7 & _T1MSMS_0 & _T1CKPS_T1CK16 & _T1OSCEN_OFF & _FCKSM_CSECME
...
TXBuffer res 16 ; Tamanho do buffer de transmissão

; Programa principal
MAIN
    ; Configurar a UART
    bsf TXSTAbits.TXEN ; Habilitar transmissão
    bsf RCSTAbits.SPEN ; Habilitar UART
    movlw 25 ; Carregar valor para cálculo de Baud Rate
    movwf SPBRG ; Definir Baud Rate para 9600 (com 4000000 ; Frequência do oscilador=4MHz)

    ; Loop principal
    movlw 0 ; Índice do buffer de transmissão
    movwf TXBufferPtr ; Inicializar o ponteiro do buffer

SEND_LOOP
    ; Carregar o próximo caractere para transmitir
    movf TXBuffer, w
    call SEND_BYTE
    ...
    movf TXBufferPtr, w
    sublw 16 ; Tamanho do buffer de transmissão
    btfss STATUS, Z
    goto SEND_LOOP
...
; Subrotina para enviar um byte pela UART
SEND_BYTE
    btfss TXSTAbits.TRMT ; Verificar se o registrador de transmissão está vazio
    goto $-1 ; Esperar até que esteja vazio

    movwf TXREG ; Enviar o byte
    return

; Mensagem para enviar
TXBufferPtr equ 0x20
TXBuffer equ 0x30
    dt "Hello, World!", 0
