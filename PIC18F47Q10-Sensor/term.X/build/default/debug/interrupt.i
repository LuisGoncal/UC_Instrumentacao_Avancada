...
_int_handler: ; when an interrupt happens, this function is called. It is your
    ;job to verify what interrupt happened and act accordingly

    BANKSEL PIR0 ;taxa de amostragem
    BTFSC PIR0, 5 ; Q:check if the timer0 interrupt flag is set (se esta a 1)->se tiver houve uma interrupção->timer no maximo-> temos de dar reset.
...
_timer0_int_handler:
    ...
    ; Q: WE CAN USE THE TIMR ((STATUS) and 0FFh), 6, a CONTROL THE ADC SAMPLING RATE STAR AN ADC CONVERSION HERE
...
_adc_int_handler:
    ...
    MOVFF ADRESH, 0 ; put the highest 8 bits of the conversion result in 0
